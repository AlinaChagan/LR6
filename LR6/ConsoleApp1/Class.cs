...
namespace Laba6
{
    ...
    struct Information
    {
        public string document; //переменные
    ...
    {
        List<Receipt> receipts = new List<Receipt>(); //список
        List<Waybill> waybills = new List<Waybill>();
        List<Check> checks = new List<Check>();

        public Receipt this[int index] //свойство для списка
        {
            get
            {
        ...
        public void AddReceipt(Receipt a) { receipts.Add(a); } //добавление элемента в  список
        public void AddWaybill(Waybill a) { waybills.Add(a); }
        public void AddCheck(Check a) { checks.Add(a); }
        public void DelReceipt(Receipt a) { receipts.Remove(a); } //удаляет элемент item из списка, и если удаление прошло успешно, то возвращает true
